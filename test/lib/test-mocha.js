...
function HTML(runner) {
  ...
  var self = this
    , stats = this.stats
    , total = runner.total
    , root = document.getElementById('test-mocha')
...
window.testMocha = require('mocha');

// boot
;(function(){
  var utils = testMocha.utils
    , options = {}

  testMocha.suite = new testMocha.Suite('', new testMocha.Context());
  ...
  testMocha.setup = function(opts){
    if ('string' === typeof opts) options.ui = opts;
    else options = opts;

    ui = testMocha.interfaces[options.ui];
    if (!ui) throw new Error('invalid mocha interface "' + ui + '"');
    if (options.timeout) testMocha.suite.timeout(options.timeout);
    ui(testMocha.suite);
    testMocha.suite.emit('pre-require', window);
  };
  ...
  testMocha.run = function(fn){
    testMocha.suite.emit('run');
    var runner = new testMocha.Runner(testMocha.suite);
    var Reporter = options.reporter || testMocha.reporters.HTML;
